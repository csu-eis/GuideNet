...
class GuideNet(nn.Module):
    ...
    def forward(self, img, lidar):
        c0_img = self.conv_img(img) # [1,32,180,240]
        c1_img = self.layer1_img(c0_img) # c1: [1,64,90,120]
        c2_img = self.layer2_img(c1_img) # c2: [1,64,45,60]
        c3_img = self.layer3_img(c2_img) # c3: [1, 256, 23, 30]
        c4_img = self.layer4_img(c3_img) # c4: [1, 256, 12, 15]
        c5_img = self.layer5_img(c4_img) # c5: [1, 256, 6, 8]
        dc5_img = self.layer5d_img(c5_img)  # c6: [1, 256, 12, 16]
        # 丢掉1
        dc5_img = dc5_img[:,:,:,:-1]
        c4_mix = dc5_img + c4_img # c4_mix: [1, 256, 12, 15]
        dc4_img = self.layer4d_img(c4_mix) 
        dc4_img = dc4_img[:,:,:-1,:] # dc4_img: [1, 256, 23, 30]
        c3_mix = dc4_img + c3_img
        dc3_img = self.layer3d_img(c3_mix)
        dc3_img = dc3_img[:,:,:-1,:] # dc3_img: [1,128,45,60]
        c2_mix = dc3_img + c2_img
        dc2_img = self.layer2d_img(c2_mix) # dc3_img: [1,64,90,120]
        c1_mix = dc2_img + c1_img # 

        c0_lidar = self.conv_lidar(lidar) # [1, 32, 180, 240]
        c1_lidar = self.layer1_lidar(c0_lidar) # [1, 64, 90, 120]
        c1_lidar_dyn = self.guide1(c1_lidar, c1_mix) # [1, 64, 90, 120]
        c2_lidar = self.layer2_lidar(c1_lidar_dyn) # [1, 128, 45, 60]
        c2_lidar_dyn = self.guide2(c2_lidar, c2_mix) # [1, 128, 45, 60]
        c3_lidar = self.layer3_lidar(c2_lidar_dyn) # [1, 256, 23, 30]
        c3_lidar_dyn = self.guide3(c3_lidar, c3_mix) # [1, 256, 23, 30]
        c4_lidar = self.layer4_lidar(c3_lidar_dyn) # [1, 256, 12, 15]
        c4_lidar_dyn = self.guide4(c4_lidar, c4_mix) # [1, 256, 12, 15]
        c5_lidar = self.layer5_lidar(c4_lidar_dyn) # [1, 256, 6, 8]
        c5 = c5_img + c5_lidar # [1, 256, 6, 8]
        dc5 = self.layer5d(c5)
        dc5 = dc5[:,:,:,:-1]
        c4 = dc5 + c4_lidar_dyn
        dc4 = self.layer4d(c4)
    ...
    def _make_layer(self, block, planes, blocks, stride=1,shape=[180,240]):
